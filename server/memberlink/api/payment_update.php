...
$phone = $_GET['phone'];
$amount = doubleval($_GET['amount']);
$email = $_GET['email'];
$name = $_GET['name'];
$membershipId = isset($_GET['membershipId']) ? intval($_GET['membershipId']) : null;
...
$signed= hash_hmac('sha256', $signing, 'your_signature');
if ($signed === $data['x_signature']) {
    ...
    // Delete all pending membership
    $sqldeletepending = "DELETE FROM `membership_purchase_tbl` WHERE 
    `payment_status` = ?";
    ...
    if (!$stmt2->execute()) {
        error_log("Failed to delete pending membership purchase: " . $stmt2->error);
    }
} else {
